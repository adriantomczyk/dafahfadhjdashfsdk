...
namespace LungCancerBayesNetwork
{
    public class LungCancerBayes
    {
        ...
        public double[] CreateStructre()
        {
            ...
            //Left
            parentProbability = CancerData.countProbabilityForParentVertex(Tdata, 3);
            Net.AddNode(Network.NodeType.Cpt, "p3");
            SetNodeStates("p3");
            Net.SetNodeDefinition("p3", parentProbability);

            parentProbability = CancerData.countProbabilityForParentVertex(Tdata, 26);
            Net.AddNode(Network.NodeType.Cpt, "p26");
            SetNodeStates("p26");
            Net.SetNodeDefinition("p26", parentProbability);

            parentProbability = CancerData.countProbabilityForParentVertex(Tdata, 38);
            Net.AddNode(Network.NodeType.Cpt, "p38");
            SetNodeStates("p38");
            Net.SetNodeDefinition("p38", parentProbability);

            //Middle
            parentProbability = CancerData.countProbabilityForParentVertex(Tdata, 13);
            Net.AddNode(Network.NodeType.Cpt, "p13");
            SetNodeStates("p13");
            Net.SetNodeDefinition("p13", parentProbability);

            parentProbability = CancerData.countProbabilityForParentVertex(Tdata, 29);
            Net.AddNode(Network.NodeType.Cpt, "p29");
            SetNodeStates("p29");
            Net.SetNodeDefinition("p29", parentProbability);

            parentProbability = CancerData.countProbabilityForParentVertex(Tdata, 41);
            Net.AddNode(Network.NodeType.Cpt, "p41");
            SetNodeStates("p41");
            Net.SetNodeDefinition("p41", parentProbability);

            //Right
            parentProbability = CancerData.countProbabilityForParentVertex(Tdata, 15);
            Net.AddNode(Network.NodeType.Cpt, "p15");
            SetNodeStates("p15");
            Net.SetNodeDefinition("p15", parentProbability);

            parentProbability = CancerData.countProbabilityForParentVertex(Tdata, 37);
            Net.AddNode(Network.NodeType.Cpt, "p37");
            SetNodeStates("p37");
            Net.SetNodeDefinition("p37", parentProbability);

            parentProbability = CancerData.countProbabilityForParentVertex(Tdata, 42);
            Net.AddNode(Network.NodeType.Cpt, "p42");
            SetNodeStates("p42");
            Net.SetNodeDefinition("p42", parentProbability);

            //Left
            childProbability = CancerData.countProbabilityDistributionForChildVertex(Tdata, 6, SetIndexes(3, 26, 38));
            Net.AddNode(Network.NodeType.Cpt, "c6");
            SetNodeStates("c6");
            Net.SetNodeDefinition("c6", parentProbability);

            childProbability = CancerData.countProbabilityDistributionForChildVertex(Tdata, 2, SetIndexes(3, 26, 38));
            Net.AddNode(Network.NodeType.Cpt, "c2");
            SetNodeStates("c2");
            Net.SetNodeDefinition("c2", parentProbability);

            //Middle
            childProbability = CancerData.countProbabilityDistributionForChildVertex(Tdata, 20, SetIndexes(13, 29, 41));
            Net.AddNode(Network.NodeType.Cpt, "c20");
            SetNodeStates("c20");
            Net.SetNodeDefinition("c20", parentProbability);

            childProbability = CancerData.countProbabilityDistributionForChildVertex(Tdata, 56, SetIndexes(13, 29, 41));
            Net.AddNode(Network.NodeType.Cpt, "c56");
            SetNodeStates("c56");
            Net.SetNodeDefinition("c56", parentProbability);

            //Right
            childProbability = CancerData.countProbabilityDistributionForChildVertex(Tdata, 19, SetIndexes(15, 37, 42));
            Net.AddNode(Network.NodeType.Cpt, "c19");
            SetNodeStates("c19");
            Net.SetNodeDefinition("c19", parentProbability);
